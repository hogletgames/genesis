...
- For MacOS `VK_ICD_FILENAMES` should point to the directory where
  `MoltenVK_icd.json`
- If Mono is installed in a custom directory, you can set `MONO_ROOT` environment variable
  to point to the root directory where `mono` binaries, such as `bin/mcs`, are located
